...
22. **Safety layers are independent of the LLM** -- Three separate guards, none of which depend on Claude behaving well. `command_safety.py` validates proposed commands against live telemetry *before* execution (`blocked` stops it, `warning` proceeds with an advisory). `command_verifier.py` polls telemetry *after* to confirm the aircraft actually changed. `validation.py` scans Claude's response text for V-speeds, altitudes, and frequencies against per-aircraft limits. `emergency.py` bypasses the LLM entirely for time-critical conditions. **The pre-execution guard now covers the fuel selector, mixture, crossfeed and parking brake in addition to gear, flaps, autopilot master and throttle** — `DEFAULT_RULES` went 7 to 13. It grew because CMD-07 registered eight previously-NACKing events in the adapter's `CommandMap`, turning a refusal into a real `TransmitClientEvent`: a rule set that does not track the reachable surface is a guard in name only, and `assisted` cannot withhold what no rule ever flagged. This is the primary reason the cascade architecture is retained over speech-to-speech — see `.planning/TECH-STACK-REVIEW.md`.
