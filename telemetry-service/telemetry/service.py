...
app = FastAPI(
    title="Airdale Telemetry Service",
    ...
    try:
        ...
        while True:
            ...
            if msg.type == "telemetry":
                ...
                await manager.route_command_ack(
                    msg.command_id, msg.success, msg.message
                )
...
async def ws_telemetry(ws: WebSocket):
    ...
    Broadcasts telemetry from all active adapters. Supports the same
    protocol as the legacy SimConnect bridge: subscribe, get_state,
    heartbeat messages.
    """
    await ws.accept()
    ...
    try:
        while True:
            ...
            if msg is None:
                # Check for legacy format (no type field, just has "position")
            ...
            if msg.type == "subscribe":
                ...
                if current:
                    state_data = current.to_legacy_simstate()
                    state_data["adapter_id"] = current.adapter_id
                    state_data["sim_name"] = current.sim_name
